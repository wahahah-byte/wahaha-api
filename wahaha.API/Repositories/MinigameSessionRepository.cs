...
{
    public MinigameSessionRepository(WahahaDbContext context, ILogger<MinigameSessionRepository> logger)
        : base(context, logger) { }

    public override async Task<IEnumerable<MinigameSession>> GetAllAsync()
    {
        _logger.LogDebug("Fetching all minigame sessions");
        return await _dbSet
    ...
    {
        _logger.LogDebug("Fetching minigame session {SessionId}", id);
        return await _dbSet
            .Include(s => s.Minigame)
            .Include(s => s.User)
    ...
    {
        _logger.LogDebug("Fetching minigame sessions for user {UserId}", userId);
        return await _dbSet
    ...
    {
        _logger.LogDebug("Fetching sessions for game {GameId}", gameId);
        return await _dbSet
    ...
    {
        _logger.LogDebug("Fetching minigame sessions with filters: UserId={UserId}, GameId={GameId}, Outcome={Outcome}",
            filters.UserId, filters.GameId, filters.Outcome);
